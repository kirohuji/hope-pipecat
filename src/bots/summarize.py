...
async def generate_conversation_summary(conversation_id: str, db=None) -> Optional[Conversation]:
    """
    ...
    try:
        ...
        messages = [msg.content for msg in await Message.find(Message.conversation_id == conversation_id).sort(Message.message_number).to_list()]
        if not messages:
        ...
        success = await update_conversation_title(conversation_id, summary)
        if not success:
