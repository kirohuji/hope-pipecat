...
import PixiCanvas from "@/components/Live2d";
...
export function ClientPage() {
  ...
  // Drawer 显示逻辑
  const showDrawer = conversationType === "text-voice" && !isFetching;
  const [drawerOpen, setDrawerOpen] = useState(false); // 默认关闭
  // 如果切换到 text-voice 以外，自动关闭 Drawer
  useEffect(() => {
    if (!showDrawer) setDrawerOpen(false);
  }, [showDrawer]);
    ...
    <RTVIClientProvider client={client!}>
      <div className="relative w-full h-screen overflow-hidden">
        ...
        <Sheet open={drawerOpen} onOpenChange={setDrawerOpen}>
          <SheetContent
            side="right"
            className="!w-[80vw] max-w-none h-screen p-0 flex flex-col overflow-y-auto"
            style={{ height: "100vh" }}
          >
            {/* 聊天内容和 Start chatting 逻辑 */}
            ...
            {/* <SheetClose className="absolute right-6 top-6 rounded-full bg-gray-200 p-2 hover:bg-gray-300 transition" /> */}
          </SheetContent>
        </Sheet>
        {/* 打开 Drawer 的按钮，仅在 text-voice 模式下显示 */}
        {conversationType === "text-voice" && !drawerOpen && (
          <Button
            className="fixed right-6 top-216 z-30 rounded-full shadow-lg bg-primary text-white hover:bg-primary/90"
            size="icon"
        ...
        {/* 聊天输入框 ChatControls 只在 text-voice 模式下主屏底部显示 */}
        {conversationType === "text-voice" && (
          <div className="fixed bottom-0 left-0 w-full z-20 bg-background">
            <ChatControls vision />
            <div className="h-4 bg-background w-full" />
          </div>
        )}
        {/* 选择会话类型和 WebSocketVoiceChat 仍然全屏显示 */}
        {(!conversationType || conversationType === undefined) && (
          <div className="absolute inset-0 z-10 flex flex-col gap-12 items-center justify-center h-full my-auto bg-white">
            ...
            <div className="grid md:grid-cols-2 gap-8 lg:gap-12 items-center justify-center">
              ...
              >
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <span className="font-light text-neutral-500">
                    Use your mic, camera and keyboard to talk with Gemini using
                    WebRTC.
                  </span>
                </div>
                <span className="opacity-50 inline-flex gap-1 items-center mt-4">
